...
def create_role():
    # 保安，负责审核帖子，所以拥有帖子和评论的权限
    inspector = RoleModel(name='保安', desc='负责审核帖子和评论是否合法合规')
    ...
    # 运营，拥有所有权限
    operator = RoleModel(name='运营', desc='负责维持网站正常运营')
...
def create_test_user():
    ...
    zhangsan = UserModel(username='张三', email='2794108@qq.com', password="123456",
                         is_staff=True, role=admin_role)

    operator_role = RoleModel.query.filter_by(name='运营').first()
    lisi = UserModel(username='李四', email='1188564@qq.com', password="123", is_staff=True, role=operator_role)

    inspector_role = RoleModel.query.filter_by(name='保安').first()
    wangwu = UserModel(username='王五', email='12456659@qq.com', password="123", is_staff=True,
                       role=inspector_role)
